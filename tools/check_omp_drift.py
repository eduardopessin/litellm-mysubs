...
def fetch_sources(version: str) -> dict[str, str]:
    """Ficheiros ``src/`` do tarball, indexados por caminho relativo."""
    url = f"{REGISTRY}/{PACKAGE}/-/pi-ai-{version}.tgz"
...
def main() -> int:
    ...
    print(f"{PACKAGE}@{OMP_VERSION}: {len(sources)} ficheiros, {len(anchors)} âncoras\n")
    ...
    if latest != OMP_VERSION:
        print(f"\naviso: {PACKAGE}@{latest} disponível (fixada: {OMP_VERSION})")
